...
**Concepts demonstrated:** multi-agent system · tool calling · guardrails · LLM-as-judge · security evals · audit trail · MCP (in-process registry + real stdio server) · Google ADK + optional Gemini · Dockerfile/Cloud Run deployability.
...
Every decision is **logged to an audit trail** and **scored by an LLM-as-judge** against the expected outcome.
...
47 synthetic cases spanning all five sources and all five decisions:
...
|---|---|---|
| Safe requests | 5 | `allow` |
| Direct prompt injection | 5 | `block` |
| Indirect injection (attachment/KB/email/tool response) | 5 | `block` ×3, `sanitize` ×2 |
| Risky legitimate actions | 3 | `require_human_approval` |
| Ambiguous | 2 | `ask_clarification` |
| Obfuscation & arg-validation (base64, markdown-exfil, refund thresholds, protected CRM field) | 5 | `block` ×4, `allow` ×1 |
...
in-process MCP-style registry + real MCP stdio server · security guardrails · deployability · LLM-as-judge · audit trail.
